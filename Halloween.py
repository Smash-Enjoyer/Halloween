...
from adafruit_led_animation.color import RED,YELLOW,ORANGE,GREEN,TEAL,CYAN,BLUE,PURPLE,MAGENTA,WHITE,BLACK,GOLD,PINK,AQUA,JADE,AMBER,OLD_LACE,RAINBOW
...
colorcycle = ColorCycle(np, 0.5, colors=RAINBOW)
...
holloween_cycle = ColorCycle(np, 0.5, colors=RAINBOW)

rainbow = Rainbow(np, speed=0.1, period=2)
rainbow_chase = RainbowChase(np, speed=0.1, size=5, spacing=3)
rainbow_comet = RainbowComet(np, speed=0.1, tail_length=7, bounce=True)
...
prideCode = AnimationSequence(rainbow,rainbow_chase,rainbow_comet,rainbow_sparkle, advance_interval=5, random_order=True)
##########holloween = AnimationSequence()
#This while true makes a lightshow based on an American theme of red white and blue.
#np.fill(red)
"orange is fOrange1 and fOrange3"
