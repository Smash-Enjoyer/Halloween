...
circuitPlayground = neopixel.NeoPixel(board.NEOPIXEL, playgroundPixels, auto_write = False, brightness = bright)
fOrange1 = (255,54,0)
fOrange3 = (255,34,0)
...
def fire(colors, speed = 0.1, times = 1):
    for i in range(times):
        ...
        for i in led:
            ...
            circuitPlayground[i] = colors[random.randint(0,2)]
            time.sleep(delay)
            circuitPlayground.show()
'''
...
def lightning(color,lColors, speed = 0.1):
    ...
    circuitPlayground.fill(color)
    circuitPlayground.show()
    time.sleep(delay)
    ind = random.randint(0, len(lColors)-1)
    for i in range(random.randint(1,4)):
        circuitPlayground.fill(lColors[ind])
        circuitPlayground.show()
        time.sleep(0.1)
        circuitPlayground.fill([0,0,0])
        circuitPlayground.show()
    circuitPlayground.fill(color)
    circuitPlayground.show()
'''
